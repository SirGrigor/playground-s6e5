...
Designed for the asymmetric interface:
...
from .observer import JSONL_PATH, _load_jsonl, add_note
...
def timeline() -> str:
    ...
    for e in rows:
        ...
        hold = e.get("holdout_auc")
        oof = e.get("oof_auc_mean")
        ...
        flag_str = ""
        if flags:
            flag_str = "  ⚠ " + ", ".join(s.split("(")[0] for s in flags)
        lines.append(
            f"{ver:<6} {par:<8} "
...
def compare(v_a: str, v_b: str) -> str:
    ...
    # Metrics
    lines.append("\n## Metrics")
    for k in ("oof_auc_mean", "holdout_auc", "runtime_sec"):
...
def regressions() -> str:
    ...
    for e in out:
        lines.append(
            f"  {e['version']} (parent={e['parent']}): holdout {e['parent_holdout_auc']:.5f} → "
            f"{e['holdout_auc']:.5f} (Δ={e['actual_delta']:+.5f})  hyp={e['hypothesis']!r}"
        )
    return "\n".join(lines) + "\n"
...
def _md_per_version(e: dict) -> str:
    ...
    lines.append(f"- **OOF AUC**: `{e.get('oof_auc_mean'):.5f}`" if e.get("oof_auc_mean") is not None else "- OOF AUC: ?")
    folds = e.get("oof_auc_per_fold") or []
    if folds:
        lines.append("- **Per-fold AUC**: " + ", ".join(f"`{x:.5f}`" for x in folds))
    lines.append(f"- **Holdout AUC**: `{e.get('holdout_auc'):.5f}`" if e.get("holdout_auc") is not None else "- Holdout AUC: ?")
    if e.get("oof_auc_mean") is not None and e.get("holdout_auc") is not None:
        gap = e["holdout_auc"] - e["oof_auc_mean"]
...
def _md_index() -> str:
    ...
    lines = [
        ...
        "",
        f"Source of truth: `experiments.jsonl` (git-tracked, append-only). Auto-regenerated by `python -m src.diary render`.",
        "",
        "## Timeline",
        "",
    ...
    lines.append("- Pitfall reference: `docs/pitfalls.md`")
    lines.append("- Reconnaissance: `docs/reconnaissance.md`")
    lines.append("")
